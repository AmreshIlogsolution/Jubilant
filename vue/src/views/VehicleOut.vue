  ...
  <div class="bg-blue w-full p-4 flex justify-center font-sans">
    <div class="rounded bg-grey-light p-2 bg-gray-500 w-screen">
      <div class="flex justify-between py-1">
        <h3 class="text-sm text-white">Vehicle Out</h3>
        <button
          @click="$router.back()"
      ...
      >
        <div v-for="asn in searchAsnVal" :key="asn.MDN_No" class="grid gap-6">
          <!-- <router-link   :to='"/asn-detail/" + asn.ASN_NO'> -->
          <!-- <router-link   :to='"/asn-detail/"'> -->
          <!-- <router-link :to="{path: 'asn-detail', {params:asn.ASN_NO}}"> -->
          <router-link
            :to="{ name: 'VehicleOutDocuments', params: { id: asn.MDN_No } }"
          >
            <div
            ...
            >
              ...
              <!-- <div class="grid grid-cols-2 gap-1 p-2">
                        <div class="text-sm text-blue-400 text-left">custId:</div>
                        <div class="text-sm">{{asn.CustID}}</div> 
                    </div> -->
      ...
      <p class="mt-3 text-white">
        <router-link :to="{ path: 'outbound' }"> To Outbound...</router-link>
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    serarchasnvalue() {
      ...
      axiosClient
        .get("/vehicle-out-invoice-search", { params: params })
        .then((response) => {
