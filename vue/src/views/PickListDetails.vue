<template>
  <div>
    ...
    <div class="bg-white p-2 rounded mt-1 h-30">
      <div>
        <h5 class="p-1 bg-slate-200 text-left text-sm">
          Pick List
          <button
            class="float-right inline-block px-1 py-1 bg-purple-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-purple-700 hover:shadow-lg focus:bg-purple-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-purple-800 active:shadow-lg transition duration-150 ease-in-out"
    ...
    >
      <div class="grid grid-cols-4 gap-6 p-2">
        <div class="text-xs text-blue-400 text-left">DN NO :</div>
        <div class="text-xs text-left">{{ pickList.dnno }}</div>

        <div class="text-xs text-blue-400 text-left">Invocie No :</div>
        <div class="text-xs text-left">{{ pickList.REF_NO }}</div>
      </div>
      <div class="grid grid-cols-4 gap-6 p-2">
        <div class="text-xs text-blue-400 text-left">Total Qty :</div>
        <div class="text-xs text-left">{{ pickList.InvQty }}</div>

        <div class="text-xs text-blue-400 text-left">Total Scan :</div>
        <div class="text-xs text-left">{{ pickList.PickQTy }}</div>
      </div>
    </div>
    ...
    <div>
      <div class="p-2 h-auto sm:rounded-md">
        <div class="flex items-center py-2 px-3 rounded-lg bg-indigo-600">
          <input
            type="text"
            v-model="zoneSearch"
          ...
          <input
            ...
            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md mx-2"
            placeholder="Search Zone *"
            @keyup.prevent="findaileshht($event)"
            autofocus
            ref="zoneRef"
          />
      ...
      <div class="px-3">
        ...
        >
          <div class="grid grid-cols-2 gap-1 p-1">
            <div class="text-xs text-blue-400 text-left ml-1">Zone:</div>
            <div class="text-xs text-left">{{ zone.ZONE }}</div>
          </div>
          <div class="grid grid-cols-4 gap-6 p-2">
            <div class="text-xs text-blue-400 text-left">Total Qty:</div>
            <div class="text-xs text-left">{{ zone.TotalQty }}</div>

            <div class="text-xs text-blue-400 text-left">Total Picked:</div>
            <div class="text-xs text-left">{{ zone.TotalScanned }}</div>
          </div>
          <div class="grid grid-cols-2 gap-1 p-1">
            <div class="text-xs text-blue-400 text-left ml-1">
              ZoneBar Code:
            </div>
            <div class="text-xs text-left">{{ zone.ZONEBARCODE }}</div>
          </div>
        </div>
      </div>
    </div>
