...
public function InsertImageQry($asnNumber,$fileName,$fileUrl,$docType,$userId,$clientId,$warehouseId){
 ...
 public function asnSearchImages(Request $request){
  ...
  if(!empty($request->warehouseId) &&  !empty($request->clientId)) {
      ...
      {
      $Rquery="SELECT P_MRN_No AS ASN_No,CustInv as InvoiceNo,VENINVOICE as VendorInvoice,convert(date,CustInvDate) as InvoiceDate,PONO FROM PreMRN_Upload WITH(NOLOCK) WHERE APFLAG='Y' AND WH='$request->warehouseId' AND CustID='$request->clientId' AND ISNULL(IMFLAG,'') ='' and  ( CustInv='$request->searchScanValue' or P_MRN_No='$request->searchScanValue') group by P_MRN_No ,CustInv,VENINVOICE ,convert(date,CustInvDate), PONO";
      }else{
      $Rquery  = "SELECT P_MRN_No AS ASN_No,CustInv as InvoiceNo,VENINVOICE as VendorInvoice,  convert(date,CustInvDate) as InvoiceDate,PONO FROM PreMRN_Upload WITH(NOLOCK) WHERE APFLAG='Y' AND WH='$request->warehouseId' AND CustID='$request->clientId' AND ISNULL(IMFLAG,'') ='' group by P_MRN_No ,CustInv,VENINVOICE ,convert(date,CustInvDate),PONO";
      }
...
// $array=["warehouseId"=>$warehouseId,"clientId"=>$clientId,""]
    ...
    //  print_r($flagUpt .'</n>'. $invoiceCheck .'</n>'.$otherdocImgcheck.'</n>'.$lrImgcheck);
    //  die();
 ...
 public function asnInwardReceiving(Request $request){
  ...
  if(!empty($request->warehouseId) &&  !empty($request->clientId)) {
   #Search Key Based on ASNNUMBER AND 
    if (!empty($request->searchScanValue)) 
    {
    $Rquery="SELECT MRNNo AS SRN_NO,InCustInvNo AS INVOICE,InFrom AS PARTYNAME,CONVERT(DATE,GRNON) AS GRNDATE FROM tbl_inward WITH(NOLOCK) WHERE WH='$request->warehouseId' AND Cust='$request->clientId' AND GRNDone='Y' AND (InCustInvNo='$request->searchScanValue' OR MRNNo='$request->searchScanValue') AND ISNULL(MRNACTIVITY,'')='' GROUP BY MRNNo ,InCustInvNo,InFrom,CONVERT(DATE,GRNON) ORDER BY CONVERT(DATE,GRNON)";
    }else{
    $Rquery  = "SELECT MRNNo AS SRN_NO,InCustInvNo AS INVOICE,InFrom AS PARTYNAME,CONVERT(DATE,GRNON) AS GRNDATE FROM tbl_inward WITH(NOLOCK) WHERE WH='$request->warehouseId' AND Cust='$request->clientId' AND GRNDone='Y' AND ISNULL(MRNACTIVITY,'')=''  GROUP BY MRNNo ,InCustInvNo,InFrom,CONVERT(DATE,GRNON) ORDER BY CONVERT(DATE,GRNON) ASC";
    }
 ...
 public function OutboundSearchImages(Request $request){
   ...
   if(!empty($request->warehouseId) &&  !empty($request->clientId)) {
      //DN-MAA-0000008
       ...
       if (!empty($request->searchScanValue)) 
       {
       $Rquery="select  MDN_No,CustInv,convert(date,MDN_Date) as MDNDate from tbl_mdn WITH(NOLOCK) where WH='$request->warehouseId' AND CustID='$request->clientId' AND mdnpost='Y' AND ISNULL(MDNACTIVITY,'')=''  AND ( MDN_No='$request->searchScanValue' OR CUSTINV='$request->searchScanValue') GROUP BY MDN_No,CustInv,convert(date,MDN_Date)   ORDER BY convert(date,MDN_Date) ASC";
       }else{
       $Rquery  = "SELECT  MDN_No,CustInv,convert(date,MDN_Date) as MDNDate from tbl_mdn WITH(NOLOCK) where WH='$request->warehouseId' AND CustID='$request->clientId' AND mdnpost='Y' AND ISNULL(MDNACTIVITY,'')='' GROUP BY MDN_No,CustInv,convert(date,MDN_Date) ORDER BY convert(date,MDN_Date) ASC";
       }
...
public function VehicleOutInvoiceSearch(Request $request){
  ...
  if(!empty($request->warehouseId) &&  !empty($request->clientId)) {
    ...
    if (!empty($request->searchScanValue)) 
    {
    $Rquery="select  MDN_No,CustInv,convert(date,MDN_Date) as MDNDate from tbl_mdn WITH(NOLOCK) where WH='$request->warehouseId' AND CustID='$request->clientId' AND mdnpost='Y' AND ISNULL(VEHIMG,'')=''  AND ( MDN_No='$request->searchScanValue' OR CUSTINV='$request->searchScanValue') GROUP BY MDN_No,CustInv,convert(date,MDN_Date)   ORDER BY convert(date,MDN_Date) ASC";
    }else{
    $Rquery  = "select MDN_No,CustInv,convert(date,MDN_Date) as MDNDate from tbl_mdn WITH(NOLOCK) where WH='$request->warehouseId' AND CustID='$request->clientId' AND mdnpost='Y' AND ISNULL(VEHIMG,'')='' GROUP BY MDN_No,CustInv,convert(date,MDN_Date) ORDER BY convert(date,MDN_Date) ASC";
    }
