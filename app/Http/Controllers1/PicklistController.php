...
class PicklistController extends Controller
{
    public function PendingPickListData(Request $request){  
        ...
        if($numrow >0){
            ...
            return response()->json(['error'=>'NoRecord']);
        }
    ...
    public function getAilesNew(Request $request){

        $queryGetAiles = "select Aisles,ISNULL(SUM(QTY),0) AS TotalQty,ISNULL(SUM(PICKQTY),0) AS TotalScanned,case when ISNULL(SUM(PICKQTY),0)=ISNULL(SUM(QTY),0)  then 1 else 0 end  as chck from Tbl_PickingDataRetail c with(nolock) left join WHLocation w with(nolock) on w.wh=c.wh and w.Location=c.WHLOCATION where c.wh='" .$request->warehouseId. "' AND C.CUSTID='" .$request->clientId. "' and REF_NO='" .$request->urlparam. "' AND ZoneBarcode='" .$request->zone. "' group by Aisles ORDER BY CHCK, Aisles ASC"; 
        
        return response($queryGetAiles);
        $aislesval = DB::select($queryGetAiles);
      
         $ailesCount = count($aislesval);
       //    //(1>0)
        
       if($ailesCount > 0){

        return response()->json($aislesval);
    ...
    public function getAiles(Request $request){
        ...
        $aislesval = DB::select($queryGetAiles);
         ...
         $ailesCount = count($aislesval);
       //  return response($ailesCount);   //(1>0)
