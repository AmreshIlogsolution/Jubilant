...
class AsnController extends Controller
{
        ...
        if(!empty($request->asnval)){         
            ...
            // function give user wise warehouse             
                ...
                DB::table('asns')
                    ...
                    ->select(['asn_subs.id as ids','asns.asn_no as asnNo',
                    'asns.invoiceNo as invoiceNo', 'asns.po_number as poNumber', 
                    'asn_subs.qty as qty','clients.clientName as cName'])                  
    ...
    public function getWareHouseName(){
       // $warehoue = DB::table('tbl_whmaster')->select('WHid','WHname')->distinct('WHname')->get();
        ...
        $query = "select distinct WHid,WHname from tbl_whmaster with(nolock) where isnull(WHactive,'')<>''";
    ...
    public function scanSku(Request $request){       
       ...
       DB::table('asns')
            ...
            ->select(['asn_subs.id as ids','asns.asn_no as asnNo',
                'asns.invoiceNo as invoiceNo', 'asns.po_number as poNumber', 'skus.skuCode as skucode',
                'asn_subs.qty as qty','clients.clientName as cName'])
                // ->where('asns.asn_no','like',$request->nameval) 
    ...
    public function getSKU(Request $request){
      ...
      $query = " SELECT COUNT(DISTINCT ITEMCODE) as totalRows, ITEMCODE AS SKU,p.batchno,isnull(sum(p.Qty),0) as ASNQTY,(select  isnull(sum(r.Qty),0) from Tbl_RetailScanning  r with(nolock) where r.wh=p.wh and r.custid=p.CUSTID and r.P_MRN_No=p.P_MRN_NO and r.invoiceno=p.custinv  AND r.sku=p.ItemCode and r.batchno=p.batchno) as ScanQty, 
       (isnull(sum(p.Qty),0)- (select  isnull(sum(r.Qty),0) from Tbl_RetailScanning  r with(nolock) where r.wh=p.wh and r.custid=p.CUSTID and r.P_MRN_No=p.P_MRN_NO and  r.invoiceno=p.custinv  AND r.sku=p.ItemCode and r.batchno = p.batchno )) as BalanceFOrScan FROM PreMRN_Upload  p WITH(NOLOCK) WHERE p.WH='".$request->warehouseId."' AND (CUSTINV='".$request->cutInvoice."') AND P.CUSTID='".$request->clientId."' AND ISNULL(p.MRN_NO,'')='' 
       ...
       if($total >0){
        return response()->json(['sku'=>$sku,'total'=>$total,'scanQtyTotalvalues'=>$scanQtyTotalvalues]);
       }else{
    ...
    public function getCleintName(){
      
        $rows = "SELECT DISTINCT custname AS ClientName,custid as Clientid FROM tbl_customer 
        WHERE ISNULL(custactive,'')=''";
   ...
   public function skuInserUpdateValues(Request $request){  
    ...
    if($numrow > 0){
        ...
        $valdate = "select isnull(sum(Qty),0) as qty,(SELECT ISNULL(SUM(QTY),0) AS ScanQTy from Tbl_RetailScanning r with(nolock) where r.wh=p.wh and r.CUSTID=p.CustID and r.sku=p.ItemCode and r.BATCHNO=p.BatchNo and r.INVOICENO =p.CustInv   ) as Scanqty from PreMRN_Upload p with(nolock) where wh='".$request->warehouseId."' and CustID='".$request->clientId."' and CustInv='".$request->invoice."' and ItemCode='".$request->skuNumber."' and BatchNo='".$request->skuBatch."' GROUP BY P.WH,P.CustID,P.ItemCode,P.BatchNo,P.CustInv"; 
         
        return response($valdate);
        $validateQuery = DB::select($valdate);
